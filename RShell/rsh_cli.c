...
int exec_remote_cmd_loop(char *address, int port)
{
    ...
    while (1) {
        ...
        if (fgets(cmd_buff, SH_CMD_MAX, stdin) == NULL) break;
        cmd_buff[strcspn(cmd_buff, "\n")] = '\0';

        if (send(cli_socket, cmd_buff, strlen(cmd_buff) + 1, 0) < 0) break;

        int received;
        while ((received = recv(cli_socket, rsp_buff, RDSH_COMM_BUFF_SZ, 0)) > 0) {
            if (rsp_buff[received - 1] == RDSH_EOF_CHAR) {
                rsp_buff[received - 1] = '\0';
                break;
            }
        }
        printf("%s\n", rsp_buff);
...
int start_client(char *server_ip, int port) {
    ...
    if (cli_socket < 0) return ERR_RDSH_CLIENT;
